...
class TrainServiceTest {
  ...
  @Mock private TrainDataServiceClient trainDataServiceClient;
  @Mock private BookingReferenceClient bookingReferenceClient;
  ...
  class ReserveTrainTests {
    ...
    void reserveSeatsOnTrain_returnReservation_withBookingReferenceForTheRequestedTrainSeats() {
      ...
      assertThat(reservation)
          .isEqualTo(new Reservation("a train", seatsToBook, "booking reference"));
    }

    @ParameterizedTest(
        name =
            "reserveSeatsOnTrain return reservation without booking reference when train service respond OK with body {0}")
    @NullAndEmptySource
    @ValueSource(strings = {"already booked with reference: 75bcd15", " "})
    void
    ...
    @ParameterizedTest(
        name =
            "reserveSeatsOnTrain return reservation without booking reference when train service respond without OK status: {0}")
    @EnumSource(
              ...
              ResponseEntity.status(status)
                  .body(
                      """
                                    ...
                                    """));
      final List<Seat> seatsToBook = List.of(new Seat("A", 10));
  ...
  class DataForTrainTests {
    ...
    void dataForTrain_returnAllSeatsInformation_forTheRequestedTrain() {
      ...
      assertThat(dataForTrain)
          .isEqualTo(
              DataForTrainFixtures.createTrain(
                      List.of(
                          new TrainSeat("A", 1, "a booking reference"),
                          new TrainSeat("A", 2, ""),
                          new TrainSeat("B", 1, ""),
                          new TrainSeat("B", 2, "another booking reference"),
                          new TrainSeat("B", 3, ""),
                          new TrainSeat("C", 1, "a third booking reference"),
                          new TrainSeat("C", 2, "")))
                  .data());
    }

    @ParameterizedTest(
        name = "dataForTrain return empty data when train service respond without OK status: {0}")
    @EnumSource(
        value = HttpStatus.class,
        names = {"OK"},
        mode = EnumSource.Mode.MATCH_NONE)
    ...
    @ParameterizedTest(
        name = "dataForTrain return empty data when train service respond OK with body {0}")
    @NullAndEmptySource
    @ValueSource(strings = {" ", "a train"})
    void dataForTrain_returnEmptyData_whenResponseIsNotDataForTrain(String body) {
      given(trainDataServiceClient.dataForTrain(anyString())).willReturn(ResponseEntity.ok(body));
